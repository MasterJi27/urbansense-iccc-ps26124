    ...
    <div>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div className="edge-mode">
          {[
            ["HIGH","EDGE_AI","RDD + ByteTrack + ANPR on trigger stills only"],
            ["MED","LIGHTWEIGHT_EDGE_AI","RDD stills; skip ANPR unless incident"],
            ["LOW","CAPTURE_AND_SENSOR","Phone IMU+GPS ~1KB JSON. No on-device YOLO."],
          ].map(([tier, mode, note]) => {
      ...
      <div className="card">
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:10,marginTop:10}}>
          {ai ? Object.entries(ai).sort(([,a],[,b])=>{ const order={REAL:0,RULE_BASED:1,EXPERIMENTAL:2,SIMULATED:3}; const sa=a?.ai_status||a?.status||"SIMULATED"; const sb=b?.ai_status||b?.status||"SIMULATED"; return (order[sa]??99)-(order[sb]??99); }).map(([k,v])=>(
            <div key={k} style={{border:"1px solid var(--line)",borderRadius:12,padding:"10px 12px",background:"var(--surface-2)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><b style={{fontSize:12}}>{k}</b><Pill status={v?.ai_status||v?.status||"SIMULATED"} /></div>
